...
void TrafficLight::cycleThroughPhases()
{
    ...
    // define cycle duration as a random value between 4 and 6 seconds
    ...
    // init stop watch
    std::chrono::time_point<std::chrono::system_clock> lastToggle;
    lastToggle = std::chrono::system_clock::now();

    // init traffic light phase
    while(true)
    {
        // sleep at every iteration to reduce CPU usage
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        long timeSinceLastToggle = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lastToggle).count();

        if (timeSinceLastToggle >= cycleDuration)
        {  
